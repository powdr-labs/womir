...
the caller's return address (RA) and the caller's frame pointer (FP), followed by the function's local
frame data, which might hold the frame for other functions called by this new function.
...
1) In the scheme proposed, the input and output slots are overlapping, so a function must necessarily overwrite some of the inputs with the outputs.
   ...
   being used, especially if it is part of the convention that input words must not be overwritten, thus the caller may be able to keep reding from
